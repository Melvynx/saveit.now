...
import { BookmarkCard, BookmarkCardLoadMore } from "@/features/app/bookmark-card";
import { SearchInput } from "@/features/app/search-input";
  ...
  return (
    ...
    >
      ...
      <div className="grid gap-4 lg:gap-6 grid-cols-[repeat(auto-fill,minmax(20rem,1fr))] [&>*]:w-full place-items-start">
        {isPending ? (
          <>
            {Array.from({ length: query ? 2 : 12 }).map((_, index) => (
              <Skeleton
                key={index}
