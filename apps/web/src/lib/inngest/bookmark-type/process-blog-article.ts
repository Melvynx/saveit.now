...
export async function processBlogArticle(
  ...
  const markdown = await step.run("convert-to-markdown", async () => {
    ...
    // Pass the cleaned content to Turndown
  ...
  // Extract the page title and metadata
  ...
  const userInput =
    markdown || screenshotDescription
      ? `${
          markdown
            ? `Here is the content in markdown of the article :
<article-content>
${markdown}
</article-content>`
            : null
...
${screenshotDescription}
  ...
  await step.run("update-bookmark", async () => {
    await updateBookmark({
      ...
      tags: getTags,
      metadata: { articleContent: markdown }, // Store clean article content for reader
      imageDescription:
