...
const POST = apiRoute.body(z.any()).handler(async (request, { ctx, body }) => {
  try {
    ...
    if (contentType.includes("multipart/form-data")) {
      const result = await parseBookmarkBody(request, ctx.user.id);
...
export const Route = createFileRoute("/api/v1/bookmarks")({
