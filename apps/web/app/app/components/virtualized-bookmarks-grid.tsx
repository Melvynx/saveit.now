...
const VirtualizedBookmarkCard = memo(
  ({
    bookmark,
    isFirstResult,
    query,
...
export function VirtualizedBookmarksGrid({
        ...
        >
          {row.map((item) => {
            if (item.type === "skeleton") {
              return (
                <Skeleton key={item.id} className="bg-muted h-72 rounded-md" />
              );
            }
  ...
  return (
    <Virtuoso
      ...
      increaseViewportBy={{ top: 200, bottom: 680 }} // Preload content and add bottom spacing
