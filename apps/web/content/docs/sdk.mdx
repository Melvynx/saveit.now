---
title: SaveIt SDK
description: Use the official `saveitnow` npm package to manage bookmarks from any Node.js, Bun, or Deno application.
order: 10
category: SDK & CLI
...
The official [`saveitnow`](https://www.npmjs.com/package/saveitnow) npm package gives you a typed client for the SaveIt.now REST API. The same package also ships an executable CLI - see the [CLI guide](/docs/cli).

## Install

```bash
npm install saveitnow
# or
pnpm add saveitnow
# or
bun add saveitnow
...
```ts
import { Saveit, SaveitApiError, SaveitConfigError } from "saveitnow";
