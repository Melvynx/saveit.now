/**
 * Marketing email content constants.
 * All strings are verbatim ports of apps/web/src/lib/inngest/marketing/emails.const.ts.
 * SITE_URL replaces getServerUrl(); APP_LINKS are hard-coded identical to current values.
 */

const getSiteUrl = () => process.env.SITE_URL ?? "https://saveit.now";
...
${getSiteUrl()}${APP_LINKS.extensions}

2. Open the app and save your first link:
${getSiteUrl()}${APP_LINKS.app}
...
export const CHROME_EXTENSION_EMAIL = () =>
  `Hi,
...
export const HOW_USE_CHROME_EXTENSION_EMAIL = () =>
  `Hi,
...
So... if you don't have the extension yet, download it here: ${getSiteUrl()}${APP_LINKS.extensions}`;

export const HOW_TO_IMPORT_BOOKMARKS_EMAIL = () =>
  `Hi,

You might have Chrome bookmarks or other tools bookmarks that you want to import.

For that you can just go to the import page: ${getSiteUrl()}${APP_LINKS.imports}
...
export const HOW_TO_USE_BOOKMARKS_EMAIL = () =>
  `Hi,
...
Start saving: ${getSiteUrl()}${APP_LINKS.app}`;

export const HOW_TO_SEARCH_BOOKMARKS_EMAIL = () =>
  `Hi,
...
Try searching for something now: ${getSiteUrl()}${APP_LINKS.app}`;

export const PREMIUM_COMMITMENT_EMAIL = () =>
  `Hi,
...
Upgrade now: ${getSiteUrl()}${APP_LINKS.upgrade}
...
export const SUBSCRIPTION_THANK_YOU_EMAIL = () =>
  `Hey,
...
export const SUBSCRIPTION_HOW_TO_USE_PREMIUM_EMAIL = () =>
  `Hi,
...
That's it! Start saving everything: ${getSiteUrl()}${APP_LINKS.app}`;

export const SUBSCRIPTION_LETS_TALK_EMAIL = () =>
  `Hi,
...
export const SUBSCRIPTION_OUR_COMMITMENT_EMAIL = () =>
  `Hi,
...
We avoid useless features, but still, we create a lot of micro improvements that you can find here: ${getSiteUrl()}${APP_LINKS.changelog}

Thanks for being pro!`;

// ---------------------------------------------------------------------------
// Limit-reached drip
// NOTE: Email says "2 days" but Stripe promo actually expires in 3 days.
//       This is a pre-existing inconsistency — preserved verbatim.
// ---------------------------------------------------------------------------

export const LIMIT_REACHED_DISCOUNT_EMAIL = (promoCode: string) =>
  `Hi,
...
<img src="${getSiteUrl()}/images/coupon.png" alt="Limit reached discount" />`;

export const LIMIT_REACHED_REMINDER_EMAIL = (promoCode: string) =>
  `Hi,
...
export const LIMIT_REACHED_LAST_CHANCE_EMAIL = (promoCode: string) =>
  `Hi,
