...
# ns-ios-deploy-app - App Store Connect deployment for NowStack Mobile
...
Build artifact generation is OS-aware: macOS may use local Xcode or local EAS, while Windows/Linux must load `ns-setup-expo` and use EAS cloud to produce the `.ipa`. App Store Connect automation belongs to `asc`.
</objective>
...
<build_ipa>
This skill needs a signed production `.ipa`. Detect the OS before choosing the build path:

```bash
uname -s || node -p "process.platform"
```

- **macOS / Darwin**: local Xcode/Expo or local EAS builds are allowed when signing/tooling are ready. Prefer local/Xcode or Xcode Cloud when the user wants no EAS cloud credits.
- **Windows/Linux**: do not attempt local iOS production builds. Load/follow `.agents/skills/ns-setup-expo/SKILL.md`, verify `eas-cli`, `eas whoami`, linked `easProjectId`, and `expo-doctor`, then use EAS cloud to produce the `.ipa`.
...
Windows/Linux required cloud path, and optional macOS fallback only if the user explicitly allows EAS cloud for build artifact generation:
