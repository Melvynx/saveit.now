...
Before choosing the build command, check the OS. Use the local `--local` build only on macOS/Darwin with Xcode available. On Windows/Linux, load/follow `.agents/skills/ns-setup-expo/SKILL.md`, verify Expo login + linked `easProjectId` + `expo-doctor`, then use an EAS cloud build without `--local`.
